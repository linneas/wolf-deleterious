...
################################################################################
# assign_genotype_to_haplotype_haploid.py
# Author: Linnéa Smeds
# Date: 22 April 2021, updated 21 June 2022 to work for X chromosome
...
alleles are associated with which haplotypes for each site. If first haplotype \
is 0, the individual is haploid for this chromosome (e.g. males on chrX)")
	...
	for line in bed:
		...
		if not s:
			...
			for h in range(3,len(tabs)):
				#haplo=re.compile("^0\|(\d+)", tabs[h])
				haplo=re.match(r"^0\|(\d+)", tabs[h])
				if haplo:
					print("we have a haploid!")
					print("haplotype was"+tabs[h])
					print("here is the group:"+haplo.group(1))
					tabs[h]=str(haplo.group(1))+'|'+str(haplo.group(1))
					print("new hap="+tabs[h])
